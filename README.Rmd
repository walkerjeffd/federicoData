...
Use `tracker_get()` to retrieve a tracker from the database. This function returns a tibble with columns `id`, `description`, `hydro`, and `wq`. The `hydro` and `wq` columns are lists containing tibbles with the dbkeys and wq stations for each tracker.
...
Use `tracker_update()` to update the data associated with each tracker. This function in turn calls `tracker_update_hydro()` and `tracker_update_wq()` to update the hydrologic and wq datasets, respectively.
