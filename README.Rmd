...
Use `db_add_dbhydro_stations()` to add stations, This function will fetch the station metadata from DBHYDRO, and will only succeed if the metadata for all specified stations is received. It will also only add metadata for stations that do not already exist in the database.
